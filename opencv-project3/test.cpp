    ...
    void graySpatialFiltering(int pic_id, int size_id=0, int select=0){
        ...
        for(int i=m; i < original_gray_image[pic_id].rows - m; i++)
            for(int j=m; j < original_gray_image[pic_id].cols - m; j++){ 
                ...
                int grad, value;   
                if(select == 0)
                    image_process.at<uchar>(i, j) = computerMeanResult(sub_matrix, size_id);
                else if(select == 1)
                    ...
                    image_grad.at<uchar>(i, j) = computerRobertResult(sub_matrix);
                else if(select == 4)
                    image_grad.at<uchar>(i, j) = computerSobelResult(sub_matrix);
                else if(select == 5)
